"""Tests for MockCourseService implementation."""

import pytest

from models.course import CourseCreate
from tests.mocks.services.course_service_mock import (
    CourseServiceMockError,
    MockCourseService,
)
...
def test_mock_service_initializes_empty(mock_service):
    """Test that MockCourseService initializes with no courses after reset."""
...
def test_create_course_with_course_create_object(mock_service):
    """Test create_course with CourseCreate object."""
    course_data = CourseCreate(name="Test Course", cs50_id=50, exercise_ids=["ex1", "ex2"])
    course = mock_service.create_course(course_data)

    assert course.name == "Test Course"
    assert course.cs50_id == 50
    assert course.exercise_ids == ["ex1", "ex2"]
...
def test_get_course_raises_exception_for_nonexistent_id(mock_service):
    ...
    with pytest.raises(CourseServiceMockError) as exc_info:
