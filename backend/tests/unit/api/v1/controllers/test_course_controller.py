...
def test_get_course_success(client):
    ...
    assert isinstance(model.cs50_id, int)
    assert isinstance(model.exercise_ids, list)
...
def test_update_course_partial_update(client):
...
def test_update_course_not_found(client):
    ...
    response = client.patch("/api/v1/courses/nonexistent", json=payload.model_dump())
    assert response.status_code == status.HTTP_404_NOT_FOUND
    data = response.json()
    assert "not found" in data["detail"].lower()
...
def test_create_and_delete_multiple_courses(client):
    course_ids = []
    for i in range(3):
    ...
    # delete all
    for cid in course_ids:
        response = client.delete(f"/api/v1/courses/{cid}")
        assert response.status_code == status.HTTP_200_OK
